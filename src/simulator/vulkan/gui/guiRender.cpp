...
namespace guib
{
	...
	//void GuiRender::preProcessWidgetTree(guib::Widget* root)
	//{
		...
		//	size = {0.1,0.1};
		//	offset = {0.3,0.3};
		//	Log::debug("GuiRender", "Setting type $0 with $1", widget->getType(), size.toString());
		//	widget->setSize(size);
		//	widget = nullptr;
	...
	void GuiRender::render(VkCommandBuffer commandBuffer)//, guib::Widget* root, std::vector<guib::Window*> windows)
	{
		guib::Offset currOffset = {0,0};
		guib::Size currSize = {1,1};
		state::clickableAreas.clear();
		state::draggableAreas.clear();
	...
	//	// Each widget render itself and call children to render
	//	widget->render(currOffset, currSize);
