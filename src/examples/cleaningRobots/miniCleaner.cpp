	...
	Object("MiniCleaner", position, rotation, {1,1,1}, 0.1f)
{
	...
	_display = new Display("Display", {0.0, 0.0, 0.0}, {0,0,0}, {.3, .3, .3}, {200,200});
	...
	Object::addChild(_display, new FixedConstraint({0,0.1,0}, {0,0,0}));
...
void MiniCleaner::run(float dt)
{
	...
	pos.z+=.1*dt*cos(glm::radians(_rotation.y));
	pos.x+=.1*dt*sin(glm::radians(_rotation.y));
	_rotation.y+=10*dt;
