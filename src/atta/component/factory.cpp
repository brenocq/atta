...
void Factory::createClones() {
    ...
    // TODO Allocate one entity of each clone
    ...
    // TODO check if it was possible to create clones properly
    _firstClone = component::createEntity();
    ...
    for (ComponentId cid : cpuDataManager->getComponents(_prototype.getId())) {
        ...
        // Copy prototype component to clones
        EntityId cloneBegin = _firstClone.getId();
        EntityId cloneEnd = _firstClone.getId() + _maxClones;
        for (EntityId clone = cloneBegin; clone < cloneEnd; clone++) {
            Component* compClone = cpuDataManager->addComponent(clone, cid);
            memcpy(compClone, compProt, componentSize);
        }
    }
}
...
void Factory::destroyClones() {
    ...
    for (int i = _maxClones * _numEntitiesCloned - 1; i >= 0; i--)
        component::destroyEntity(_firstClone.getId() + i);
