...
#include <atta/event/events/createEntityEvent.h>
#include <atta/event/events/deleteComponentEvent.h>
#include <atta/event/events/deleteEntityEvent.h>
...
Component* ComponentManager::addEntityComponentByIdImpl(ComponentId id, EntityId entity) {
    ...
    if (e->components[compReg->getIndex()] != nullptr) {
        LOG_WARN("ComponentManager", "Could not add component [w]$1[] to entity [w]$0[]. The entity [w]$0[] already has the component [w]$1[]",
...
std::vector<EntityId> ComponentManager::getEntitiesViewImpl() { return std::vector<EntityId>(_entities.begin(), _entities.end()); }

std::vector<EntityId> ComponentManager::getNoPrototypeView() { return getInstance().getNoPrototypeViewImpl(); }
std::vector<EntityId> ComponentManager::getNoPrototypeViewImpl() { return std::vector<EntityId>(_noPrototypeView.begin(), _noPrototypeView.end()); }

std::vector<EntityId> ComponentManager::getCloneView() { return getInstance().getCloneViewImpl(); }
std::vector<EntityId> ComponentManager::getCloneViewImpl() { return std::vector<EntityId>(_cloneView.begin(), _cloneView.end()); }
...
std::vector<EntityId> ComponentManager::getScriptViewImpl() { return std::vector<EntityId>(_scriptView.begin(), _scriptView.end()); }
