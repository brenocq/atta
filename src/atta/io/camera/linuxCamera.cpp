...
bool LinuxCamera::setFps(unsigned fps) {
    ...
    if (param.parm.capture.timeperframe.numerator) {
        double newFps = param.parm.capture.timeperframe.denominator / param.parm.capture.timeperframe.numerator;
...
bool LinuxCamera::openDevice() {
    ...
    if (_fd == -1) {
        LOG_WARN("io::LinuxCamera", "Could not open camera file '$0'. Error $1: $2", _deviceName, errno, strerror(errno));
        return false;
    }
    return true;
}
...
bool LinuxCamera::initDevice() {
    ...
    for (size_t i = 0; i < _buffers.size(); i++) {
        ...
        _buffers[i].start =
            mmap(NULL /* start anywhere */, buf.length, PROT_READ | PROT_WRITE /* required */, MAP_SHARED /* recommended */, _fd, buf.m.offset);
