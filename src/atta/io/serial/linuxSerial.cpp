...
LinuxSerial::~LinuxSerial() { close(_fd); }
...
bool LinuxSerial::start() {
    ...
    if (ioctl(_fd, TCGETS2, &_tty) != 0) {
        LOG_WARN("io::LinuxSerial", "[*w](start)[] Could not initialize termios struct for [w]$0[]. Error($1): $2", _deviceName, errno,
                 std::strerror(errno));
        return false;
    }
    ...
    if (!setBaudRate(_baudRate)) {
        LOG_WARN("io::LinuxSerial", "[*w](start)[] Could not configure termios struct for [w]$0[]. Error($1): $2", _deviceName, errno,
                 std::strerror(errno));
        return false;
    }
...
int LinuxSerial::receive(uint8_t* buf, uint16_t size) { return read(_fd, buf, size); }

void LinuxSerial::transmit(uint8_t* buf, uint16_t size) { write(_fd, buf, size); }
...
bool LinuxSerial::setBaudRate(unsigned baudRate) {
    ...
    if (ioctl(_fd, TCSETS2, &_tty) != 0) {
        LOG_WARN("io::LinuxSerial", "[*w](setBaudRate)[] Could not configure termios struct for [w]$0[]. Error($1): $2", _deviceName, errno,
                 std::strerror(errno));
        return false;
    }
