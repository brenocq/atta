...
int LinuxBluetooth::bluezParseDevice1(sd_bus_message* m, const char* opath, Device* dev, LinuxDevice* lDev) {
    ...
    while ((r = sd_bus_message_enter_container(m, 'e', "sv")) > 0) {
        ...
        if (strcmp(str, "Name") == 0) {
            ...
            r = snprintf(lDev->path.data(), lDev->path.size() + 1, "%s/dev_%02X_%02X_%02X_%02X_%02X_%02X", _adapter.c_str(), dev->mac[5], dev->mac[4],
                         dev->mac[3], dev->mac[2], dev->mac[1], dev->mac[0]);
            if (r < 0) {
