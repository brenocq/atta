...
namespace atta
{
    PoolAllocator::PoolAllocator(size_t blockCount, size_t minBlockSize, size_t blockAlign):
        ...
        _blockSize(std::max((size_t)sizeof(Node), minBlockSize)),
        _blockCount(blockCount),
        _blockAlign(blockAlign == 0 ? _blockSize : blockAlign),
        _freeList(nullptr)
    {
    ...
    {
        return (reinterpret_cast<uint8_t*>(block) - _memory)/_blockSize;
    }

    void* PoolAllocator::getBlock(uint64_t index)
    {
        return reinterpret_cast<void*>(_memory+index*_blockSize);
    }
