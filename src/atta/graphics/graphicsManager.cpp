...
void GraphicsManager::pushLayerImpl(Layer* layer) { _layerStack->push(layer); }
...
EntityId GraphicsManager::viewportEntityClick(std::shared_ptr<Viewport> viewport, vec2i pos) {
    return getInstance().viewportEntityClickImpl(viewport, pos);
}
EntityId GraphicsManager::viewportEntityClickImpl(std::shared_ptr<Viewport> viewport, vec2i pos) { return _computeEntityClick->click(viewport, pos); }
