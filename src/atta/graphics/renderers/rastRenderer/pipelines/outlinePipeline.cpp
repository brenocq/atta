...
namespace atta::vk
{
		...
		Pipeline(vkCore, imageViews, scene)
	{
		...
		//---------- Shaders ----------//
		_vertShaderModule = std::make_shared<ShaderModule>(_device, "src/shaders/shaders/outlineShader.vert.spv");
		_fragShaderModule = std::make_shared<ShaderModule>(_device, "src/shaders/shaders/outlineShader.frag.spv");
