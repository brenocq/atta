...
fs::path FileSelectionWindow::getChosenFile() { return getInstance()._chosenFile; }
...
void FileSelectionWindow::renderImpl() {
    ...
    ImGui::SameLine();
    // Show current path
    if (ImGui::InputText("Current path###FileSelectionWindowCurrentPath", _currPathBuffer.data(), _currPathBuffer.size(),
